...
class ratingspageastakermy extends StatefulWidget {
  const ratingspageastakermy({Key? key}) : super(key: key);

  @override
  State<ratingspageastakermy> createState() => _ratingspageastakermyState();
}

class _ratingspageastakermyState extends State<ratingspageastakermy> {
  ...
  ListView buildreviewsonmyjob(BuildContext ctx) {
    List<Widget> w = [];
    for (int i = 0; i < astakerlistmy.length; i++) {
      w.add(containerreviewsonmyjob(
        ctx: ctx,
        id: astakerlistmy[i]['reviewid'],
        jobId: astakerlistmy[i]['jobId'],
        jobName: astakerlistmy[i]['jobName'],
        byUserUsername: astakerlistmy[i]['byUserUsername'],
        byUserUsernameImageUrl: astakerlistmy[i]['byUserUsernameImageUrl'],
        onUserUsername: astakerlistmy[i]['onUserUsername'],
        onUserUsernameImageUrl: astakerlistmy[i]['onUserUsernameImageUrl'],
        type: astakerlistmy[i]['type'],
        rating: astakerlistmy[i]['rating'],
        publishDate: "${astakerlistmy[i]["updateDate"] !="" ?"Edited: ":""}" + "${astakerlistmy[i]["updateDate"] !="" ? astakerlistmy[i]["updateDate"].toString().substring(0,10):astakerlistmy[i]["publishDate"]}",
        updateDate: astakerlistmy[i]['updateDate'],
        reviewText: astakerlistmy[i]['reviewText'],
        usersht: userbool,
      ));
      setState(() {
        userbool!=userbool;
