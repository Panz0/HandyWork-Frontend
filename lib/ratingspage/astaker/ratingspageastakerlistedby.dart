...
class ratingspageastakerlistedby extends StatefulWidget {
  const ratingspageastakerlistedby({Key? key}) : super(key: key);

  @override
  State<ratingspageastakerlistedby> createState() => _ratingspageastakerlistedbyState();
}

class _ratingspageastakerlistedbyState extends State<ratingspageastakerlistedby> {
  ...
  ListView buildreviewsonmyjob(BuildContext ctx) {
    List<Widget> w = [];
    for (int i = 0; i < astakerlistlistedby.length; i++) {
      w.add(containerreviewsonmyjob(
        ctx: ctx,
        id: astakerlistlistedby[i]['reviewid'],
        jobId: astakerlistlistedby[i]['jobId'],
        jobName: astakerlistlistedby[i]['jobName'],
        byUserUsername: astakerlistlistedby[i]['byUserUsername'],
        byUserUsernameImageUrl: astakerlistlistedby[i]['byUserUsernameImageUrl'],
        onUserUsername: astakerlistlistedby[i]['onUserUsername'],
        onUserUsernameImageUrl: astakerlistlistedby[i]['onUserUsernameImageUrl'],
        type: astakerlistlistedby[i]['type'],
        rating: astakerlistlistedby[i]['rating'],
        publishDate: "${astakerlistlistedby[i]["updateDate"] !="" ?"Edited: ":""}" + "${astakerlistlistedby[i]["updateDate"] !="" ? astakerlistlistedby[i]["updateDate"].toString().substring(0,10):astakerlistlistedby[i]["publishDate"]}",
        updateDate: astakerlistlistedby[i]['updateDate'],
        reviewText: astakerlistlistedby[i]['reviewText'],
        usersht: userbool,
      ));
      setState(() {
        userbool!=userbool;
